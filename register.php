<?php
    ...
    $m='';              // declare a variable to store message
    if(isset($_POST['submit'])){
        $name= $_POST['name'];
        $uName= $_POST['uname'];
        $email= $_POST['email']?$_POST['email']:''; //if no value then empty string
        $pass= $_POST['pass'];
        $rPass= $_POST['r_pass'];
        if($pass===$rPass){         //check if the password match
...
<html>
    ...
    <body>
        <form method="POST" action="register.php">   <!-- We don't want to combine values with url, so post method is used -->
            <div class="container reg">

                <span><?php if($m!='') echo $m; ?></span>   <!-- To show the messege -->
